...
from . import processor, filter, translator, summarizer
from .models import EmailNotification, AccountStatus
...
async def connect_all(
    ...
    Args:
        ...
        cfg: App config (translate, summary, merge_batch, merge_interval, filters)
    ...
    # Merge timer
    merge_tasks = []
    if cfg.get('merge_batch', True):
        merge_tasks.append(asyncio.create_task(
            _merge_timer(cfg.get('merge_interval', 30), on_email)
        ))
    ...
    while True:
        ...
        for name in list(_merge_queue):
            if not _merge_queue[name]:
                continue
            # Flush accounts that have been waiting > interval
                ...
                if data and data[0]:
                    ...
                    if current_max > last_uid:
                        ...
                        if backlog_emails:
                            ...
                            merge = cfg.get('merge_batch', True)
                            if merge and len(backlog_emails) > 1:
...
async def _build_and_push(
    ...
    translate_enabled = cfg.get('translate', False)
    summary_enabled = cfg.get('summary', True)
    filters_cfg = cfg.get('filters', {})

    for sender, subject, body, atts in emails:
        # Filter
        if filter.should_filter(sender, subject, body, filters_cfg):
...
async def _build_merged(
    ...
    for sender, subject, body, atts in batch:
        if not filter.should_filter(sender, subject, body, filters_cfg):
    ...
    for i, (sender, subject, body, atts) in enumerate(filtered[:5]):
        ...
        if translate_enabled and not re.search(r'[\u4e00-\u9fff]', subject):
            ...
            if cn and cn != subject:
                lines.append(f'   🌐 {cn}')
        summary_data = summarizer.extract(body) if cfg.get('summary', True) else None
